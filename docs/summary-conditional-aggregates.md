...
| `src/lib/summary/line-rewardcard-conditional-allowlist.ts` | LINE リワードカード系の **UI / Zod と共有するホワイトリスト**（テーブル名・数値列） |
...
### `line_oam_rewardcard_table_cond_agg`（B: ホワイトリスト UI）

- **意味**: `line_oam_rewardcards` に紐づく **許可テーブル**について、**対象日**の行のうち **比較列が条件を満たす行**をリワードカード横断で集約する。
- **集約 `aggregate`**:
  - `sum`: **`sumField` 列を合算**（`compareField` と同じ列は不可）
  - `row_count`: **条件を満たす行数**（`sumField` 不要）
- **許可テーブル・数値列**: `src/lib/summary/line-rewardcard-conditional-allowlist.ts` の `LINE_REWARDCARD_COND_AGG_TABLE_SPEC` を唯一のソースとする。テーブルを増やすときは **このファイルと Zod の `table` enum・fetch の分岐**を揃える。
- **params スキーマ**: `LineRewardcardTableCondAggParamsSchema`（`summary-conditional-definitions.ts`）

LINE サービス詳細のカスタム指標モーダル「**条件付き集計（LINE）**」はこの `definitionId` を保存する。

### `line_oam_shopcard_point_cond_sum`（レガシー）

- 旧保存で残っている **`definitionId` 専用**。フェッチ時は `resolveSummaryFormulaDataRef` が **`line_oam_rewardcard_table_cond_agg` と同じ仮想 ref** に正規化する。
- 新規保存では使わない（UI は `line_oam_rewardcard_table_cond_agg` のみ出力）。

## 旧形式（互換）

`line_oam_shopcard_point.cumulative_users@{op}:{threshold}` 形式の **`cumulativeUsersSliceRef`** は、フェッチ時に **`line_oam_rewardcard_table_cond_agg`**（`table=line_oam_shopcard_point`, `point` 条件, `users` 合算）の仮想 ref に正規化される。
