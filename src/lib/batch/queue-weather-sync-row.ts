...
import { deadLetterBatchJob, failBatchJob } from '@/lib/batch/batch-queue'
import {
...
export type WeatherHandlerResult = { ok: true } | { ok: false; error: string; permanent?: boolean }
...
export async function processWeatherSyncRow(
  admin: SupabaseClient,
  row: BatchJobQueueRow
): Promise<WeatherHandlerResult> {
  if (!row.project_id) {
