...
export type LineOamDailyBatchResult = {
  success: boolean
  target_date?: string
  processed?: number
  errors?: number
...
export async function runLineOamDailyBatch(
  admin: SupabaseClient,
  opts: { serviceId: string | null }
): Promise<LineOamDailyBatchResult> {
  const startedAt = new Date()
  const serviceFilter = opts.serviceId ?? undefined
  ...
  try {
    ...
    if (!configs || configs.length === 0) {
      ...
      return { success: true, processed: 0, message: 'No active LINE OAM services' }
    }
