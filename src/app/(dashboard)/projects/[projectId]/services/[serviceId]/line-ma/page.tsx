...
export default function LineMaConnectionPage({
  ...
  const { data: credData, mutate } = useSWR<{ success?: boolean; data?: CredentialsState; error?: string }>(
    service?.service_type === 'line' ? `/api/services/${serviceId}/line-messaging/credentials` : null,
    fetcher,
  )
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">LINE Messaging MA</h1>
          <p className="text-sm text-gray-400">{service?.service_name ?? ''}</p>
        </div>
      </div>

      <p className="text-xs text-gray-500 mb-4">
        <Link href={`/projects/${projectId}/services/${serviceId}/line`} className="text-green-600 hover:underline">
          LINE OAM（bot_id・リワードカード）
        </Link>
        {' · '}
        <Link href={`/projects/${projectId}/services/${serviceId}/line/dashboard`} className="text-green-600 hover:underline">
          LINE OAM ダッシュボード
        </Link>
      </p>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
        <h2 className="font-bold text-gray-900 mb-2">接続状態（UI-1）</h2>
        <p className="text-xs text-gray-500 mb-4">
          Messaging API の Channel secret と long-lived Channel access token を保存します（サーバー側で暗号化）。
        </p>
        ...
        <form onSubmit={saveCredentials} className="space-y-3 max-w-xl">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Channel access token（long-lived）</label>
            <input
              type="password"
              autoComplete="off"
              value={channelToken}
