...
interface GoogleAdsCredentialInfo {
  id: string
  auth_status: 'pending' | 'active' | 'error'
  google_account_email: string | null
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {activeTab === 'settings' && (
        <>
          ...
          {/* Google 広告 連携設定 */}
          <GoogleAdsCredentialSection clientId={clientId} />
...
// --------------------------------------------------------------------------
// Google Ads 連携設定セクション
// --------------------------------------------------------------------------
function GoogleAdsCredentialSection({ clientId }: { clientId: string }) {
  const { data: credData, mutate: mutateCred } = useSWR<{ success: boolean; data: GoogleAdsCredentialInfo | null }>(
    `/api/clients/${clientId}/google-ads/credential`, fetcher
  )
  const credential = credData?.data
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!oauthClientId.trim() || !oauthClientSecret.trim() || !managerCustomerId.trim()) {
      setError('Client ID / Client Secret / MCC（manager_customer_id）を入力してください')
      return
    }
    setSaving(true)
    setError('')
    const res = await fetch(`/api/clients/${clientId}/google-ads/credential`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        oauth_client_id: oauthClientId.trim(),
        oauth_client_secret: oauthClientSecret.trim(),
        manager_customer_id: managerCustomerId.trim(),
      }),
    })
    const json = await res.json()
    if (!json.success) { setError(json.error ?? '保存に失敗しました'); setSaving(false); return }
    ...
    setOauthClientId('')
    setOauthClientSecret('')
    setManagerCustomerId('')
    setSaving(false)
    mutateCred()
  }

  const handleDelete = async () => {
    if (!confirm('Google 広告の連携設定を削除しますか？OAuth認証情報もすべて削除されます。')) return
    await fetch(`/api/clients/${clientId}/google-ads/credential`, { method: 'DELETE' })
    mutateCred()
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-100 to-sky-100 flex items-center justify-center text-lg">📣</div>
          <h2 className="font-bold text-gray-900">Google 広告 連携設定</h2>
        </div>
        {credential && (
          <button onClick={handleDelete} className="text-xs text-gray-400 hover:text-red-500 transition">
            設定を削除
      ...
      {(showForm || (!credential && false)) && (
        <form onSubmit={handleSave} className="space-y-3 mt-2">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">
              login-customer-id（通常は MCC の顧客ID）
            </label>
            <input
              type="text"
            ...
            />
            <p className="text-xs text-gray-400 mt-1">
              子アカウントのデータを取るときは、その子を管理している<strong className="text-gray-600">管理アカウント（MCC）の10桁</strong>を入れてください。
              管理アカウントがない場合は、サービス設定の広告アカウント customer_id と<strong className="text-gray-600">同じ10桁</strong>を入れてください。
            </p>
          </div>
          {error && <p className="text-sm text-red-600">{error}</p>}
          <div className="flex gap-2">
      ...
      {credential && (
        <div className="space-y-3">
          <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
            ...
            >
              Client ID/Secret/MCCを変更
            </button>
          </div>

          <div className="bg-gray-50 rounded-xl px-4 py-3">
            <p className="text-xs text-gray-400 mb-0.5">login-customer-id（登録値・通常は MCC）</p>
            <p className="text-sm font-mono font-medium text-gray-800">{credential.manager_customer_id}</p>
          </div>

          {credential.auth_status !== 'active' && (
            <a
              href={`/api/clients/${clientId}/google-ads/auth`}
              className="flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-white border-2 border-gray-300 rounded-xl text-sm font-medium text-gray-700 hover:border-blue-400 hover:text-blue-700 transition"
            >
              <svg className="w-4 h-4" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
