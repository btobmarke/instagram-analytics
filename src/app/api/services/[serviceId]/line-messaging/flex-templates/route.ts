...
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  const { data, error } = await admin
    ...
    .select('id, name, template_kind, created_at, updated_at')
...
const PostSchema = z.object({
  ...
  template_kind: z.enum(['flex', 'carousel']).optional().default('flex'),
  template_json: z.record(z.string(), z.any()),
})
...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      template_kind: parsed.data.template_kind,
